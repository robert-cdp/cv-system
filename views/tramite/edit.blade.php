...
@section('content')
    <div class="card card-outline card-primary fade-in">
        ...
        <form method="POST" action="{{ route('tramite.update', $tramite->id) }}" class="ajax-service-form">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                        <x-form.select name="status" label="Estado" icon="fas fa-certificate" :options="[
                            ...
                            placeholder="-- Seleccione --" value="{{ $tramite->status }}"/>
                    </div>

                    {{-- Campo adicional 1 --}}
                    <div class="col-12 col-md-6">
                        <x-form.textarea name="field_additional_1" label="Campo Adicional 1" icon="fas fa-pencil-alt"
                            rows="2" value="{{ $tramite->field_additional_1 }}" />
                    </div>

                    {{-- Campo adicional 2 --}}
                    <div class="col-12 col-md-6">
                        <x-form.textarea name="field_additional_2" label="Campo Adicional 2" icon="fas fa-pencil-alt"
                            rows="2" value="{{ $tramite->field_additional_2 }}" />
                    </div>

                    {{-- Comentario --}}
                    <div class="col-12">
                        <x-form.textarea name="comment" label="Comentario" icon="fas fa-comment" rows="5" value="{{ $tramite->comment }}" />
                    </div>
                </div>
            </div>
